...
operandFromSymbol (symbol * sym)
{
  ...
  if (sym->level && sym->etype && SPEC_OCLS (sym->etype) &&
      (IN_FARSPACE (SPEC_OCLS (sym->etype)) && !TARGET_HC08_LIKE && !TARGET_M6502_LIKE &&
      (!(options.model == MODEL_FLAT24))) && options.stackAuto == 0)
    {
      ok = 0;
    }
  ...
  if (!IS_AGGREGATE (sym->type) &&      /* not an aggregate */
      ...
      !((TARGET_HC08_LIKE || TARGET_M6502_LIKE) && (getSize (sym->type) > 2)) && /* will fit in regs */
      ok                                /* farspace check */
    )
    {
...
geniCodeRValue (operand * op, bool force)
{
  ...
  if (!IS_ITEMP (op) && !force && !(IN_FARSPACE (SPEC_OCLS (etype)) && !TARGET_HC08_LIKE && !TARGET_M6502_LIKE))
    {
  ...
  if (IS_SPEC (type) &&
      IS_TRUE_SYMOP (op) && (!(IN_FARSPACE (SPEC_OCLS (etype)) && !TARGET_HC08_LIKE && !TARGET_M6502_LIKE) || (options.model == MODEL_FLAT24)))
    {
      op = operandFromOperand (op);
      op->isaddr = 0;
      return op;
...
geniCodeMultiply (operand * left, operand * right, RESULT_TYPE resultType)
{
  ...
  if ((p2 > 0) && !IS_FLOAT (letype) && !IS_FIXED (letype) &&
      !((resultType == RESULT_TYPE_INT) && (getSize (resType) != getSize (ltype)) && !(TARGET_Z80_LIKE || TARGET_M6502_LIKE || TARGET_IS_STM8 && p2 == 1) /* Mimic old behaviour that tested port->muldiv, which was zero for stm8 and z80-like only. Someone should look into what really makes sense here. */) &&
...
/* if the right is a literal & power of 2 and left is unsigned then
  ...
  /* if the right is a literal & power of 2
     ...
     followed by right shift */
      ...
      ((p2 = powof2 ((TYPE_TARGET_ULONG) ulFromVal (OP_VALUE (right)))) > 0) &&
      (TARGET_Z80_LIKE || TARGET_HC08_LIKE || TARGET_M6502_LIKE))
    {
      operand *tmp;
          ...
          if (!sym->addrtaken && !IS_VOLATILE (sym->etype))
            {
              ...
              if ((IN_FARSPACE (SPEC_OCLS (sym->etype)) && !TARGET_HC08_LIKE && !TARGET_M6502_LIKE) &&
...
geniCodeCritical (ast * tree, int lvl)
{
  ...
  if (!options.stackAuto && !TARGET_HC08_LIKE && !TARGET_M6502_LIKE)
    {
