...
#define IS_AOP_HX(x) ((x)->regmask == M6502MASK_HX)
  ...
  while (n)
    {
      if (n > 127)
      ...
      else
        {
          ...
          else if (n == 1 && optimize.codeSize && m6502_reg_h->isFree)
            {
...
loadRegFromAop (reg_info * reg, asmop * aop, int loffset)
{
  ...
  if (m6502_reg_h->aop && m6502_reg_h->aop->op && aop->op
      && operandsEqu (m6502_reg_h->aop->op, aop->op) && (m6502_reg_h->aopofs == loffset))
    {
      DD (emitcode ("", "; found correct value for %s in h", reg->name));
      transferRegReg (m6502_reg_h, reg, FALSE);
      m6502_useReg (reg);
      return;
    }
  ...
  switch (regidx)
    ...
    case XA_IDX:
      if (IS_AOP_XA (aop))
        ...
        transferRegReg (m6502_reg_hx, m6502_reg_xa, FALSE);
...
loadRegHXAfromAop(asmop * aopH, int ofsH, asmop * aopX, int ofsX, asmop * aopA, int ofsA)
{
  ...
  if (aopA && aopX && IS_AOPOFS_X (aopA, ofsA) && IS_AOPOFS_A (aopX, ofsX))
    {
      ...
      pushReg (m6502_reg_a, TRUE);
      if (aopH)
        loadRegFromAop (m6502_reg_h, aopH, ofsH);
      transferRegReg (m6502_reg_x, m6502_reg_a, FALSE);
      pullReg (m6502_reg_x);
      return;
    }
  if (aopA && aopH && IS_AOPOFS_H (aopA, ofsA) && IS_AOPOFS_A (aopH, ofsH))
    {
      ...
      pushReg (m6502_reg_a, TRUE);
      if (aopX)
        loadRegFromAop (m6502_reg_x, aopX, ofsX);
  ...
  if (aopX && aopH && IS_AOPOFS_H (aopX, ofsX) && IS_AOPOFS_X (aopH, ofsH))
    {
      ...
      pushReg (m6502_reg_x, TRUE);
      if (aopA)
        loadRegFromAop (m6502_reg_a, aopA, ofsA);
  ...
  if (aopA && aopH && aopX && IS_AOPOFS_A (aopX, ofsX) && IS_AOPOFS_X (aopH, ofsH) && IS_AOPOFS_H (aopA, ofsA))
    {
      ...
      pushReg (m6502_reg_a, FALSE);
      transferRegReg (m6502_reg_h, m6502_reg_a, FALSE);
      transferRegReg (m6502_reg_x, m6502_reg_h, FALSE);
      pullReg (m6502_reg_x);
      return;
    }
  ...
  if (aopX && (aopX->type == AOP_REG) && !IS_AOP_WITH_X (aopX))
    {
      loadRegFromAop (m6502_reg_x, aopX, ofsX);
      if (aopA)
        m6502_freeReg (m6502_reg_a); /* in case it's needed to load H */
      if (aopH)
        loadRegFromAop (m6502_reg_h, aopH, ofsH);
      if (aopA)
        loadRegFromAop (m6502_reg_a, aopA, ofsA);
  ...
  if (aopH && (aopH->type == AOP_REG) && !IS_AOP_WITH_H (aopH))
    {
      if (aopA)
        m6502_freeReg (m6502_reg_a); /* in case it's needed to load H */
      if (aopX)
        m6502_freeReg (m6502_reg_x); /* in case it's needed to load H */
      loadRegFromAop (m6502_reg_h, aopH, ofsH);
      if (aopX)
        loadRegFromAop (m6502_reg_x, aopX, ofsX);
      if (aopA)
  ...
  if (aopH && aopX && (aopH == aopX) && (ofsH == (ofsX+1)))
    loadRegFromAop (m6502_reg_hx, aopX, ofsX);
  else
    {
      if (aopH)
        loadRegFromAop (m6502_reg_h, aopH, ofsH);
...
forceStackedAop (asmop * aop, bool copyOrig)
{
  ...
  if (copyOrig && m6502_reg_a->isFree)
    reg = m6502_reg_a;
  else if (copyOrig && m6502_reg_x->isFree)
    reg = m6502_reg_x;
  else
    reg = NULL;
...
storeRegToAop (reg_info *reg, asmop * aop, int loffset)
{
  ...
  if ((reg->rIdx == HX_IDX) && aop->stacked && (aop->stk_aop[loffset] || aop->stk_aop[loffset + 1]))
    {
      storeRegToAop (m6502_reg_h, aop, loffset + 1);
  ...
  switch (regidx)
    ...
    case HX_IDX:
      ...
      if (aop->type == AOP_DIR || IS_M65C02 && aop->type == AOP_EXT)
        {
          emitcode ("sthx", "%s", aopAdrStr (aop, loffset, TRUE));
          regalloc_dry_run_cost += (aop->type == AOP_DIR ? 2 : 3);;
        }
    ...
    case XA_IDX:
      if (IS_AOP_HX (aop))
        transferRegReg (reg, m6502_reg_hx, FALSE);
      else if (IS_AOP_XA (aop))
        break;
      else if (IS_AOP_AX (aop))
        {
  ...
  //if (!reg->aop || (reg->aop && (reg->aop != aop)))
  {
    ...
    if ((!m6502_reg_x->aop || !m6502_reg_h->aop) && m6502_reg_hx->aop)
      {
        m6502_reg_hx->aop = NULL;
  ...
  if (reg->rIdx == HX_IDX)
    {
      ...
      m6502_reg_h->isLitConst = 1;
      m6502_reg_h->litConst = (c >> 8) & 0xff;
    }
  m6502_useReg (reg);
}
  ...
  switch (regidx)
    ...
    case HX_IDX:
      if (size == 1)
        ...
        {
          storeRegToAop (reg, aop, 0);
          storeRegSignToUpperAop (m6502_reg_h, aop, 2, isSigned);
        }
      break;
    case XA_IDX:
      if (size == 1)
...
loadRegIndexed (reg_info * reg, int offset, char * rematOfs)
{
  ...
  switch (reg->rIdx)
    ...
    case XA_IDX:
      ...
      loadRegIndexed (m6502_reg_x, offset, rematOfs);
      break;
    default:
      wassert (0);
    }
      ...
      if (!sym->isspilt)
        {
          for(i = 0; i < sym->nRegs; i++)
            if (sym->regs[i] == m6502_reg_h || sym->regs[i] == m6502_reg_x)
              return TRUE;
        }
    }
...
aopOpExtToIdx(asmop * result, asmop *left, asmop *right)
{
  ...
  if (!m6502_reg_x->isFree || !m6502_reg_h->isFree)
    return;
  ...
  if (winner->op && right && right->op && winner->op->key == right->op->key)
    right->type = AOP_IDX;
  loadRegFromImm (m6502_reg_hx, winner->aopu.aop_dir);
}
      ...
      else if (IS_AOP_H (aop))
        {
          if (m6502_reg_a->isFree)
          ...
          else
            {
              ...
              emitcode ("tst", "1,s");
              regalloc_dry_run_cost += 4;
              if (IS_M65C02 && optimize.codeSpeed)
                {
                ...
                {
                  emitcode ("pulh", ""); // 1 Byte, 2 (m6502) / 3 (m65c02) cycles.
...
genCopy (operand * result, operand * source)
{
  ...
  while (size)
    {
      if (size >= 2 && m6502_reg_h->isDead && m6502_reg_x->isDead  &&
        (AOP_TYPE (source) == AOP_IMMD || AOP_TYPE (source) == AOP_LIT ||IS_M65C02 && AOP_TYPE (source) == AOP_EXT) &&
        (AOP_TYPE (result) == AOP_DIR || IS_M65C02 && AOP_TYPE (result) == AOP_EXT))
        {
          loadRegFromAop (m6502_reg_hx, AOP (source), offset - 1);
          storeRegToAop (m6502_reg_hx, AOP (result), offset - 1);
          offset -= 2;
          size -= 2;
        }
  ...
  if (AOP_TYPE (IC_RESULT (ic)) == AOP_REG && AOP_TYPE (IC_LEFT (ic)) == AOP_REG)
    {
      while (size--)
        {
          if ((reg = AOP (IC_RESULT (ic))->aopu.aop_reg[offset]) != m6502_reg_h)
            {
            ...
            {
              if ((reg = AOP (IC_LEFT (ic))->aopu.aop_reg[offset]) == m6502_reg_h)
...
//  l = aopGet (AOP (IC_LEFT (ic)), 0, FALSE, TRUE);
  if (AOP_TYPE (IC_LEFT (ic)) == AOP_IMMD || AOP_TYPE (IC_LEFT (ic)) == AOP_LIT ||IS_AOP_HX (AOP (IC_LEFT (ic))))
    {
      if ((size == 2) && m6502_reg_hx->isDead || IS_AOP_HX (AOP (IC_LEFT (ic))))
        {
          loadRegFromAop (m6502_reg_hx, AOP (IC_LEFT (ic)), 0);
          ...
          if (sym->regsUsed)
            {
              ...
              for (i = sym->regsUsed->size; i >= 0; i--)
                {
                  if (bitVectBitValue (sym->regsUsed, i) || (m6502_ptrRegReq && (i == HX_IDX || i == HX_IDX)))
...
genPlusIncr (iCode * ic)
{
  ...
  if ((IS_AOP_HX (AOP (left)) || IS_AOP_HX (AOP (result)) ||
    ((AOP_TYPE (left) == AOP_DIR || IS_M65C02 && AOP_TYPE (left) == AOP_EXT) && (AOP_TYPE (result) == AOP_DIR || IS_M65C02 && AOP_TYPE (result) == AOP_EXT))) &&
    (icount >= -128) && (icount <= 127) && (size == 2))
    {
      needpulx = pushRegIfSurv (m6502_reg_x);
      needpulh = pushRegIfSurv (m6502_reg_h);
      loadRegFromAop (m6502_reg_hx, AOP (left), 0);
      emitcode ("aix", "#%d", icount);
      regalloc_dry_run_cost += 2;
      m6502_dirtyReg (m6502_reg_hx, FALSE);
      storeRegToAop (m6502_reg_hx, AOP (result), 0);
      pullOrFreeReg (m6502_reg_h, needpulh);
      pullOrFreeReg (m6502_reg_x, needpulx);
      return TRUE;
    }
  if (size == 1 && (IS_AOP_X (AOP (result)) && (!IS_AOP_A (AOP (left)) || m6502_reg_h->isDead) || IS_AOP_X (AOP (left)) && !IS_AOP_A (AOP (result)) && m6502_reg_x->isDead && m6502_reg_h->isDead))
    {
  ...
  if (size == 1 || (lUnsigned && rUnsigned))
    {
      ...
      emitcode ("mul", "");
      regalloc_dry_run_cost++;
      m6502_dirtyReg (m6502_reg_xa, FALSE);
      storeRegToFullAop (m6502_reg_xa, AOP (result), TRUE);
...
genCmp (iCode * ic, iCode * ifx)
{
  ...
  if (size == 1 && IS_AOP_X (AOP (left)))
  ...
  else if (size == 1 && IS_AOP_A (AOP (left)))
    {
      ...
      && ((AOP_TYPE (right) == AOP_LIT) || ((AOP_TYPE (right) == AOP_DIR || IS_M65C02 && AOP_TYPE (right) == AOP_EXT) && (AOP_SIZE (right) == 2))) && (m6502_reg_h->isDead && m6502_reg_x->isDead || IS_AOP_HX (AOP (left))))
    {
      loadRegFromAop (m6502_reg_hx, AOP (left), 0);
      emitcode ("cphx", "%s", aopAdrStr (AOP (right), 0, TRUE));
      regalloc_dry_run_cost += (AOP_TYPE (right) == AOP_DIR ? 2 : 3);
      m6502_freeReg (m6502_reg_hx);
    }
  else
    {
      offset = 0;
...
genCmpEQorNE (iCode * ic, iCode * ifx)
{
  ...
  if ((size == 2)
      && ((AOP_TYPE (left) == AOP_DIR || IS_AOP_HX (AOP (left))) && (AOP_SIZE (left) == 2))
      && ((AOP_TYPE (right) == AOP_LIT) || ((AOP_TYPE (right) == AOP_DIR || IS_M65C02 && AOP_TYPE (right) == AOP_EXT) && (AOP_SIZE (right) == 2))) && m6502_reg_h->isDead && m6502_reg_x->isDead)
    {
      loadRegFromAop (m6502_reg_hx, AOP (left), 0);
      emitcode ("cphx", "%s", aopAdrStr (AOP (right), 0, TRUE));
      regalloc_dry_run_cost += (AOP_TYPE (right) == AOP_DIR ? 2 : 3);
...
genLeftShift (iCode * ic)
{
  ...
  emitcode (countreg == m6502_reg_a ? "tsta" : (countreg ? "tstx" : "tst 1, s"), "");
...
genPointerGet (iCode * ic, iCode * pi, iCode * ifx)
{
      ...
      if (AOP_SIZE (result) == 1)
        {
          if (!pi)
          ...
          else
            {
              ...
              m6502_dirtyReg (m6502_reg_hx, FALSE);
              m6502_freeReg (m6502_reg_x);
              storeRegToAop (m6502_reg_hx, AOP (IC_RESULT (pi)), 0);
              pi->generated = 1;
              storeRegToAop (m6502_reg_a, AOP (IC_RESULT (ic)), 0);
            }
        }
          ...
          if (pi || (IS_AOP_XA (AOP (IC_RESULT (ic))) && vol))
            {
              loadRegIndexed (m6502_reg_a, litOffset, rematOffset);
              pushReg (m6502_reg_a, FALSE);
              loadRegIndexed (m6502_reg_a, litOffset+1, rematOffset);
              m6502_useReg (m6502_reg_a);
              if (pi)
                {
                  emitcode ("aix", "#%d", size);
          ...
          else
            {
              if (IS_AOP_HX (AOP (IC_RESULT (ic))))
                {
                  m6502_freeReg (m6502_reg_x);
                  loadRegIndexed (m6502_reg_hx, litOffset, rematOffset);
                }
              else if (IS_AOP_XA (AOP (IC_RESULT (ic))))
                {
                  loadRegIndexed (m6502_reg_a, litOffset+1, rematOffset);
                  loadRegIndexed (m6502_reg_x, litOffset, rematOffset);
                }
              else if (IS_AOP_AX (AOP (IC_RESULT (ic))))
                {
                  loadRegIndexed (m6502_reg_a, litOffset, rematOffset);
                  loadRegIndexed (m6502_reg_x, litOffset+1, rematOffset);
                }
            }
        }
  ...
  else if (!pi && !ifx && size == 2 && IS_M65C02 && m6502_reg_x->isDead && m6502_reg_h->isDead && AOP_TYPE (result) == AOP_EXT) /* Todo: Use this for bigger sizes, too */
    {
      loadRegIndexed (m6502_reg_hx, litOffset, rematOffset);
      storeRegToAop (m6502_reg_hx, AOP (result), 0);
    }
      ...
      while (size--)
        {
          ...
          loadRegIndexed (m6502_reg_a, xoffset, rematOffset);
          if (!ifx)
            storeRegToAop (m6502_reg_a, AOP (result), offset);
          offset--;
        }
...
genDataPointerSet (operand * left, operand * right, operand * result, iCode * ic)
{
  ...
  if (IS_AOP_HX (AOP (right)))
    {
      storeRegToAop (m6502_reg_hx, derefaop, 0);
    }
  else
    {
      while (size--)
  ...
  if (IS_BITVAR (retype) || IS_BITVAR (letype))
  ...
  else
    {
      ...
      while (offset--)
        {
          loadRegFromAop (m6502_reg_a, AOP (right), offset);
          storeRegIndexed (m6502_reg_a, litOffset + size - offset - 1, rematOffset);
          m6502_freeReg (m6502_reg_a);
        }
    }
...
genAssignLit (operand * result, operand * right)
{
  ...
  if ((AOP_TYPE (result) != AOP_DIR ) && IS_M6502)
    canUseHX = FALSE;
      ...
      for (offset=size-2; offset>=0; offset -= 2)
        {
          if (assigned[offset] || assigned[offset+1])
            continue;
          if (m6502_reg_hx->isLitConst && m6502_reg_hx->litConst == ((value[offset+1] << 8) + value[offset]))
            {
              storeRegToAop (m6502_reg_hx, AOP (result), offset);
              assigned[offset] = 1;
              assigned[offset+1] = 1;
            }
        }
      ...
      for (offset=size-2; offset>=0; offset -= 2)
        {
          if (assigned[offset] && assigned[offset+1])
            continue;
          loadRegFromConst (m6502_reg_hx, (value[offset+1] << 8) + value[offset]);
          storeRegToAop (m6502_reg_hx, AOP (result), offset);
          assigned[offset] = 1;
          assigned[offset+1] = 1;
        }
  ...
  if (AOP_SIZE (result) == 2 && AOP (result)->type == AOP_REG)
    {
      if (IS_AOP_HX (AOP (result)) && (AOP_SIZE (right) == 2))
        {
          loadRegFromAop (m6502_reg_hx, AOP (right), 0);
          goto release;
        }
  ...
  while (size)
    {
      ...
      else if ((size > 2 || size >= 2 && !signExtend) && m6502_reg_h->isDead && m6502_reg_x->isDead && 
        (AOP_TYPE (right) == AOP_IMMD || IS_M65C02 && AOP_TYPE (right) == AOP_EXT) &&
        (AOP_TYPE (result) == AOP_DIR || IS_M65C02 && AOP_TYPE (result) == AOP_EXT))
        {
          loadRegFromAop (m6502_reg_hx, AOP (right), offset);
          storeRegToAop (m6502_reg_hx, AOP (result), offset);
          offset += 2;
          size -= 2;
        }
...
/*---------------------------------------------------------------------------------------*/
/* genm6502iode - generate code for M6502 based controllers for a single iCode instruction */
/*---------------------------------------------------------------------------------------*/
static void
genm6502iCode (iCode *ic)
{
  ...
  switch (ic->op)
    ...
    case GET_VALUE_AT_ADDRESS:
      genPointerGet (ic, hasIncm6502 (IC_LEFT (ic), ic, getSize (operandType (IC_RESULT (ic)))), ifxForOp (IC_RESULT (ic), ic));
      break;

    case '=':
      if (POINTER_SET (ic))
        genPointerSet (ic, hasIncm6502 (IC_RESULT (ic), ic, getSize (operandType (IC_RIGHT (ic)))));
      else
        genAssign (ic);
      break;
  ...
  for (ic = lic; ic; ic = ic->next)
    {
      ...
      if (!m6502_reg_h->isFree)
        DD (emitcode ("", "; forgot to free h"));
      if (!m6502_reg_hx->isFree)
